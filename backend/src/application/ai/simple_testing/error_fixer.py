"""
Error Fixing - Use AI with automatic fallback (Groq → Gemini → Mistral)
"""
import json
import random
import string
...
async def fix_payload_from_error(
    ...
    """Fix payload based on error using AI with automatic fallback"""
    ai = get_ai_provider()
...
"""
    ...
    result_text, provider = ai.generate(
        prompt=prompt,
        system_prompt="Fix API payloads based on error responses. Return ONLY valid JSON."
    )
    
    if not result_text:
        print(f"  [WARN] All AI providers failed for error fixing")
        print(f"[OK] Fixed payload:\n{{}}")
        return {}
    
    print(f"[OK] Error fix generated using {provider.value.upper()}")
